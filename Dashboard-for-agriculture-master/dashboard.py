...
if not st.sidebar.checkbox("Most Produced Commodities", True, key='2'):
...
if not st.sidebar.checkbox("Value of Agriculture production", True, key='3'):
    select_region=st.sidebar.selectbox('region',['India','Morocco','Russian Federation'],key=3)
    select_item=st.sidebar.selectbox('Item',items,key=4)
    data=load_valueData()
    x=data[data["Item"]==select_item]
...
if not st.sidebar.checkbox("Mean Temperature Change ", True, key='4'):
    select_region=st.sidebar.selectbox('region',['India','Morocco','Russian Federation'],key=6)
    select_time=st.sidebar.selectbox('month',month,key=7)
